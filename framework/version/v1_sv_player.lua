...
function AMA.LoadPlayer(source, identifier)
    MySQL.single('SELECT * FROM ama_players WHERE identifier = ?', {identifier}, function(result)
        ...
        if result then
            -- Joueur existant
            playerData.uuid = result.uuid
            playerData.wallet_uuid = result.wallet_uuid
            ...
            playerData.bitcoin = result.bitcoin or 0
            playerData.job = result.job
            playerData.job_grade = result.job_grade
            playerData.crew = result.crew or "none"
            playerData.crew_grade = result.crew_grade or 0
            ...
            -- Nouveau joueur - Générer UUID et Wallet UUID
            playerData.uuid = AMA.GenerateUUID()
            playerData.wallet_uuid = AMA.GenerateUUID()
            playerData.money = Config.Player.StartMoney
            playerData.bank = Config.Player.StartBank
            playerData.bitcoin = Config.Player.StartBitcoin
            playerData.job = Config.Player.DefaultData.job
            playerData.job_grade = Config.Player.DefaultData.job_grade
            playerData.crew = Config.Player.DefaultData.crew
            playerData.crew_grade = Config.Player.DefaultData.crew_grade
            playerData.group = Config.Player.DefaultData.group
            
            MySQL.insert([[
                INSERT INTO ama_players 
                (identifier, uuid, wallet_uuid, money, bank, bitcoin, job, job_grade, crew, crew_grade, `group`) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ]], {
                identifier, 
                playerData.uuid, 
                playerData.wallet_uuid,
                playerData.money, 
                playerData.bank,
                playerData.bitcoin,
                playerData.job, 
                playerData.job_grade,
                playerData.crew,
                playerData.crew_grade,
                playerData.group
            })
            
            AMA.Log("INFO", "Nouveau joueur créé: " .. GetPlayerName(source))
            AMA.Log("INFO", "UUID: " .. playerData.uuid .. " | Wallet: " .. playerData.wallet_uuid)
...
function AMA.SavePlayer(source)
    ...
    local canSave = AMA.TriggerHook("ama:hook:beforeSave", source, xPlayer)
    ...
    AMA.TriggerHook("ama:hook:afterSave", source, xPlayer)
